...
<div class="d-flex flex-column mb-5 fv-row">
    <!--begin::Label-->
    <label class="d-flex align-items-center fs-5 fw-bold mb-2">
        <span class="required">{{trans_choice('content.category', 1)}}</span>
        <i class="fas fa-exclamation-circle ms-2 fs-7" data-bs-toggle="tooltip" title="Select User Roles"></i>
    </label>
    <!--end::Label-->
    <!--begin::Select-->
    {{Form::select('category_id', $categories, null, ['id' =>'category_id','class' => 'form-select form-select-solid','placeholder'=>'---Please Select---'])}}
...
{!! JsValidator::formRequest('App\Http\Requests\SubCategoryRequest','form') !!}
